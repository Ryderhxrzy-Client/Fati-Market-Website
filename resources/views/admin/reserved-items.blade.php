...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h3 class="text-lg font-bold text-gray-900">Reserved Items</h3>
            <p class="text-sm text-gray-600">Items that have been reserved by buyers</p>
        </div>
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-sm font-semibold text-gray-900">Reserved Date</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($items as $item)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                <p class="text-sm text-gray-900">{{ $item['buyer_name'] ?? 'Pending' }}</p>
                            </td>
                            <td class="px-6 py-4">
                                <p class="text-sm font-medium text-green-600">${{ number_format($item['price'] ?? 0, 2) }}</p>
                            </td>
                            <td class="px-6 py-4">
                                <p class="text-sm text-gray-600">{{ isset($item['reserved_at']) ? date('M d, Y', strtotime($item['reserved_at'])) : 'N/A' }}</p>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex gap-2">
                                    ...
                                    <button class="px-3 py-1 text-sm font-medium text-green-600 hover:bg-green-50 rounded transition" title="Mark as sold">
                                        <i class="fas fa-check-circle"></i>
                                    </button>
                                    <button class="px-3 py-1 text-sm font-medium text-red-600 hover:bg-red-50 rounded transition" title="Cancel reservation">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                ...
                                <p class="text-gray-500">No reserved items found</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
